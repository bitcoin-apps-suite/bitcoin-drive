...
const DevSidebar: React.FC<DevSidebarProps> = ({ onCollapsedChange }) => {
  ...
  useEffect(() => {
    // Fetch GitHub issue count for bitcoin-drive
    fetch('https://api.github.com/repos/bitcoin-apps-suite/bitcoin-drive/issues?state=open')
      .then(res => res.json())
  ...
  const menuItems: Array<{
    ...
    { divider: true },
    { path: 'https://github.com/bitcoin-apps-suite/bitcoin-drive', icon: GitBranch, label: 'GitHub', external: true },
    { path: 'https://github.com/bitcoin-apps-suite/bitcoin-drive/issues', icon: Bug, label: 'Issues', external: true, badge: issueCount > 0 ? String(issueCount) : undefined },
    { path: 'https://github.com/bitcoin-apps-suite/bitcoin-drive/pulls', icon: Code, label: 'Pull Requests', external: true },
    { divider: true },
